...
export default function ProfilePage() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      ...
      {user?.hasPasswordProvider !== false && (
        <div className="card mt-6 flex items-center justify-between p-7">
          <div>
            <h2 className="font-display text-lg font-semibold text-chalk">Password</h2>
            <p className="mt-1 text-sm text-graphite">Update the password you use to sign in.</p>
          </div>
          <Link to="/change-password" className="btn-primary w-fit shrink-0">
            Change password
          </Link>
        </div>
      )}
