...
export default function LandingPage() {
  ...
  // The navbar/footer links use "/#features" etc. React Router doesn't
  // scroll to the fragment on its own (that's native browser behavior,
  // which only fires on a full page load) - so do it ourselves whenever
  // the hash changes, whether that's a fresh navigation to "/" or a
  // same-page click while already on the landing page.
  useEffect(() => {
    if (!location.hash) return
    const id = location.hash.slice(1)
    // Wait a tick so the section is in the DOM before we measure/scroll to it.
  ...
  return (
    <main>
      {/* Hero */}
      <section id="home" className="mx-auto max-w-6xl px-6 pb-20 pt-14 scroll-mt-24 md:pt-20">
      ...
      {/* Features */}
      <section id="features" className="scroll-mt-24 border-t border-panelBorder/60 bg-surface/40">
        <div className="mx-auto max-w-6xl px-6 pb-4 pt-20">
          <h2 className="font-display text-2xl font-semibold text-chalk">Why Sketch2Real</h2>
          <p className="mt-2 max-w-lg text-base leading-relaxed text-graphite">
            Everything you need to take a rough sketch to a finished, photoreal render — without
            leaving the browser.
          </p>
        </div>
      </section>
      ...
      {/* How it works */}
      <section id="how-it-works" className="scroll-mt-24 border-t-0 border-panelBorder/60 bg-surface/40">
