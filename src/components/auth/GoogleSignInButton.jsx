...
export default function GoogleSignInButton({ label = 'Continue with Google', onError, onSuccess }) {
  ...
  async function handleClick() {
    if (!isFirebaseConfigured) {
      onError?.('Firebase isn\u2019t configured yet - add the VITE_FIREBASE_* keys to .env.')
      return
    }
    ...
    try {
      ...
      if (message) onError?.(message) // null message = user closed the popup, not a real error
    } finally {
